            ...
            if train:
                ...
                for i in range(len(ims)):
                    ...
                    for j in range(len(temp_image)):
                        temp_image[j, :, :] = (temp_image[j, :, :] - torch.min(temp_image[j, :, :])) / (
                                    torch.max(temp_image[j, :, :] - torch.min(temp_image[j, :, :])) + .0000001)
        ...
        if self.dir is None:
            ...
            if self.transform:
                return self.transform(
                    torch.from_numpy(cv2.imread(f"{self.dir}/image{idx}.tif", cv2.IMREAD_UNCHANGED).T / 255).float())
...
def uncertianty_penalty(predictions, alpha=.05):
    return 0
    return torch.mean(.25 - torch.pow(predictions - .5, 2)) * alpha
